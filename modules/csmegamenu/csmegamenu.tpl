...
<div class="cs_mega_menu" id="menu">	
	...
	{foreach from=$menus item=menu  name=menus}
		<li class="{$menu->classes} menu_item{if $smarty.foreach.menus.first} menu_first{/if} {if $smarty.foreach.menus.last}menu_last {/if}level-1{if $menu->options && $menu->options|@count > 0} parent{/if}">
			...
			{/if}
			<a class="title_menu_parent" href="{$menu->link_of_title[(int)$cookie->id_lang]}">
				 ...
				 {if isset($option['category_parent'])}
					...
					{if isset($option['sub_category']) && $option['sub_category']}
						{assign var="sub_categories" value=$option['sub_category']['children']}
							...
							<ul class="column level_0" style="width : {$menu->width_item}px;">
								...
								{if isset($option['product_list'][$ul])}
									...
									<div class="center_block">
										{if $option['content_option']->opt_show_image_product ==  1      } 											
											<div class="image">
											<a class="product_image_menu" title="{$product.name|escape:'htmlall':'UTF-8'}" href="{$product.link}">
										...
										<h3><a alt="{$product.name|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}" href="{$product.link}">{$product.name|escape:'htmlall':'UTF-8'|truncate:40:'...'}</a></h3></div>
...
<div id="megamenu-responsive">
    <ul id="megamenu-responsive-root">
        <li class="menu-toggle"><p></p>{l s='Navegación' mod='csmegamenu'}</li>
        <li class="root">
            {$responsive_menu}
        </li>
    </ul>
