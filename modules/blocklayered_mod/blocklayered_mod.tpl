									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered_mod'}"><i class="icon-remove"></i></a>
													{if $filter.format == 1}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered_mod'}"><i class="icon-remove"></i></a>
														{$filter.name|escape:'html':'UTF-8'}{l s=':' mod='blocklayered_mod'} {$value.name|escape:'html':'UTF-8'}
													</li>
												{/if}
											{/foreach}
										{/if}
					...
					{if isset($filter.values)}
						...
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 toogle_content layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
							{if !isset($filter.slider)}
								...
								{assign var='layeredIterator' value=0}
									{foreach from=$filter.values key=id_value item=value name=fe}

										{if $value.nbr || !$hide_0_values}
										<li class="nomargin {if $layeredIterator >= $filter.filter_show_limit}hiddable{/if} col-xs-12">
										...
										{assign var='layeredIterator' value=$layeredIterator+1}
										{/if}
									{/foreach}
								{else}
									...
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
										<li class="nomargin {if $layeredIterator >= $filter.filter_show_limit}hiddable{/if}">
											{if isset($filter.is_color_group) && $filter.is_color_group}
							...
							{assign var='layeredIterator' value=0}
								...
								{/if}
							{/if}
							{if $layeredIterator > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
								<span class="hide-action more"><i class="icon-double-angle-down"></i> {l s='Show more' mod='blocklayered_mod'}</span>
								<span class="hide-action less"><i class="icon-double-angle-up"></i> {l s='Show less' mod='blocklayered_mod'}</span>
							{/if}
						</ul>
					</div>
					{/if}
