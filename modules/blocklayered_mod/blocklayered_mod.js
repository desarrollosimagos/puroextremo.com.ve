...
$(document).ready(function()
{
	...
	$(document).on('click', '#layered_form label.layered_color', function(e){
			e.preventDefault();
			var disable = $(this).parent().find('input').attr('disabled');
			if (disable == ''
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if (result.pagination.search(/[^\s]/) >= 0) {
				...
				var pagination_bottom = $('<div/>').html(result.pagination);

				//console.log(result.pagination);
				//console.log(result.pagination_bottom);
				...
				if ($('<div/>').html(pagination_bottom).find('#pagination').length)
				{
					$('#pagination_bottom').show();
					$('#pagination_bottom').html(pagination_bottom.find('#pagination').html());
				}
				else
				{
			...
			{	
				e.preventDefault();
				val = $('div.pagination input[name=n]').first().val();
				$('select[name=n]').children().each(function(it, option) {
					if (option.value == val)
						$(option).attr('selected', true);
					else
						{	
							$('select[name=n]').append('<option value="' + val + '" selected="selected">' + val + '</option>');
							return false;
						}
				});
