			...
			api =  jQuery(".{if $option->fullwidth == 'on'}fullwidthbanner{else}banner{/if}").revolution(
				{
					...
					fullWidth:"{$option->fullwidth}"							// Turns On or Off the Fullwidth Image Centering in FullWidth Modus
						});
					{if $option->soloarrowrightvalign == "bottom" || $option->soloarrowleftvalign == "bottom"}
					$(".tparrows").addClass("topauto");
