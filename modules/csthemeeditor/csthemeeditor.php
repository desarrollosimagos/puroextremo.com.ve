...
class CsThemeEditor extends Module
{
	...
	function __construct()
	{
		...
		if(is_array(glob($this->path_pattern."*.jpg")) && is_array(glob($this->path_pattern."*.gif")) &&is_array(glob($this->path_pattern."*.png")))
		{
	...
	function install()
	{
		return (parent::install() AND $this->registerHook('copyright') AND $this->registerHook('header') AND Configuration::updateValue('CS_COLOR_TEMPLATE','default',false,Shop::getGroupFromShop(Configuration::get('PS_SHOP_DEFAULT')),Configuration::get('PS_SHOP_DEFAULT')));
	}
	...
	function hookHeader($params)
	{
		...
		$this->context->controller->addJs($this->_path.'js/frontend/csjquery.cookie.js');
		$this->context->controller->addCss($this->_path.'css/colorpicker.css', 'all');
		$this->context->controller->addJs($this->_path.'js/colorpicker.js');
		$this->context->controller->addJs($this->_path.'js/eye.min.js');
		$this->context->controller->addJs($this->_path.'js/utils.js');
