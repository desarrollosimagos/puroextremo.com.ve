	...
	<div id="pb-left-column">
		...
		<form id="buy_block" target="_parent" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<label>{l s='Cantidad:' mod='csquickview'}</label>
			...
			<p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<span id="availability_label">{l s='Disponibilidad:' mod='csquickview'}</span>
			...
			<p id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
				...
				<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='artículos disponibles' mod='csquickview'}</span>
			</p>
			{/if}
		...
		<div class="content_prices clearfix">
			...
			{else}
				<p id="add_to_cart" class="buttons_bottom_block">
					<span></span>
					<input type="submit" name="Submit" onclick="return closeFancy()" value="{l s='Añadir a la cesta' mod='csquickview'}" class="exclusive" />
				</p>
			{/if}
