...
class HomepageAdvertise extends Module {
	
	private $_postErrors = array();
	
	function __construct() {
		$this->name = 'homepageadvertise';
		...
		parent::__construct();
		$this->displayName = $this->l('Hompage adverts');
		$this->description = $this->l('Display image adverts on homepage');
	}
	
	public function install()
	{
		$this->_clearCache('homepageadvertise.tpl');
		...
		if (!Configuration::updateValue($this->name.'_links'	, '')		OR
			...
			!Configuration::updateValue($this->name.'_number'	, 3)	OR
	...
	public function getContent() {
		...
		<script type="text/javascript" src="../modules/'.$this->name.'/homepageadvertise.js"></script>
	...
	function hookHome($params) {
		if (!$this->isCached('homepageadvertise.tpl', $this->getCacheId()))
		{
			global $smarty;
		...
		return $this->display(__FILE__, 'homepageadvertise.tpl', $this->getCacheId());
	...
	{	
		if (!isset($this->context->controller->php_self) || $this->context->controller->php_self != 'index')
			return;
		$this->context->controller->addCSS(($this->_path).'homepageadvertise.css', 'all');
	}
