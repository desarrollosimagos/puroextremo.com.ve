...
*}
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html'}">{l s='Mi cuenta'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Mis vales'}{/capture}
...
<table class="discount std table_block">
	<thead>
		<tr>
			<th class="discount_code first_item">{l s='Codigo'}</th>
			<th class="discount_description item">{l s='Descripción'}</th>
			<th class="discount_quantity item">{l s='Cantidad'}</th>
			<th class="discount_value item">{l s='Valor'}*</th>
			<th class="discount_minimum item">{l s='Mínimo'}</th>
			<th class="discount_cumulative item">{l s='Acumulativo'}</th>
			<th class="discount_expiration_date last_item">{l s='Fecha de expiración'}</th>
		</tr>
	</thead>
	<tbody>
		...
		<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
			...
			<td class="discount_value">
				...
				{elseif $discountDetail.id_discount_type == 2}
					{convertPrice price=$discountDetail.value} ({if $discountDetail.reduction_tax == 1}{l s='Impuestos incluidos'}{else}{l s='Impuestos no incluidos'}{/if})
			...
			<td class="discount_cumulative">
				{if $discountDetail.cumulable == 1}
					<img src="{$img_dir}icon/yes.gif" alt="{l s='Si'}" class="icon" /> {l s='Si'}
				{else}
...
{else}
	<p class="warning">{l s='Usted no tiene ningún vales.'}</p>
{/if}
