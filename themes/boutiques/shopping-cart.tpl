...
{capture name=path}{l s='Su cesta de compra'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1 id="cart_title">{l s='Resumen Cesta-Compra'}</h1>
...
{if isset($empty)}
	<p class="warning">{l s='Su cesta de compra está vacía.'}</p>
{elseif $PS_CATALOG_MODE}
	<p class="warning">{l s='Esta tienda no ha aceptado su nuevo pedido.'}</p>
{else}
	...
	var txtProduct = "{l s='producto' js=1}";
	var txtProducts = "{l s='productos' js=1}";
	var deliveryAddress = {$cart->id_address_delivery|intval};
	// ]]>
	</script>
	<p style="display:none" id="emptyCartWarning" class="warning">{l s='Su cesta de compra está vacía.'}</p>
{if isset($lastProductAdded) AND $lastProductAdded}
	<div class="cart_last_product">
		<div class="cart_last_product_header">
			<div class="left">{l s='Último producto añadido'}</div>
		</div>
...
{/if}
<p>{l s='Su cesta de compra contiene:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='producto'}{else}{l s='productos'}{/if}</span></p>
	...
	<table id="cart_summary" class="std">
		<thead>
			<tr>
				...
				<th class="cart_unit item">{l s='Precio por unidad'}</th>
		...
		{if $use_taxes}
			{if $priceDisplay}
				<tr class="cart_total_price">
					<td colspan="5">{if $display_tax_label}{l s='Total productos (iva excl.)'}{else}{l s='Total productos'}{/if}</td>
					<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
				</tr>
			{else}
				<tr class="cart_total_price">
					<td colspan="5">{if $display_tax_label}{l s='Total productos (iva incl.)'}{else}{l s='Total productos'}{/if}</td>
					<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products_wt}</td>
				</tr>
			{/if}
		{else}
			<tr class="cart_total_price">
				<td colspan="5">{l s='Total productos'}</td>
				<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
			</tr>
		{/if}
			<tr{if $total_wrapping == 0} style="display: none;"{/if}>
				<td colspan="5">
				{if $use_taxes}
					{if $display_tax_label}{l s='Total envoltura de regalo (iva incl.):'}{else}{l s='Total envoltura de regalo cost:'}{/if}
				{else}
					{l s='Total envoltura de regalo cost:'}
				{/if}
				</td>
			...
			{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
				<tr class="cart_total_delivery" style="{if !isset($carrier->id) || is_null($carrier->id)}display:none;{/if}">
					<td colspan="5">{l s='Envío'}</td>
					<td colspan="2" class="price" id="total_shipping">{l s='Envío gratis!'}</td>
				</tr>
			{else}
				{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
					{if $priceDisplay}
						<tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
							<td colspan="5">{if $display_tax_label}{l s='Total envío (iva excl.)'}{else}{l s='Total envío'}{/if}</td>
					...
					{else}
						<tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
							<td colspan="5">{if $display_tax_label}{l s='Total envío (iva incl.)'}{else}{l s='Total envío'}{/if}</td>
							<td colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping}</td>
						</tr>
					{/if}
				{else}
					<tr class="cart_total_delivery"{if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
						<td colspan="5">{l s='Total envío'}</td>
						<td colspan="2" class="price" id="total_shipping" >{displayPrice price=$total_shipping_tax_exc}</td>
					</tr>
				{/if}
			{/if}
			<tr class="cart_total_voucher" {if $total_discounts == 0}style="display:none"{/if}>
				...
				{if $display_tax_label}
					{if $use_taxes && $priceDisplay == 0}
						{l s='Total vales (iva incl.):'}
					{else}
						{l s='Total vales (iva excl.)'}
					{/if}
				{else}
					{l s='Total vales'}
				{/if}
			...
			<tr class="cart_total_price">
				<td colspan="5">{l s='Total (iva excl.)'}</td>
				<td colspan="2" class="price" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</td>
			</tr>
			<tr class="cart_total_tax">
				<td colspan="5">{l s='Total iva'}</td>
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" class="product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval}{if $odd} odd{else} even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
						...
						<td class="cart_quantity" colspan="2">
							...
							{else}
								...
								<a rel="nofollow" class="cart_quantity_up" id="cart_quantity_up_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery}&amp;id_customization={$id_customization}&amp;token={$token_cart}")}" title="{l s='Añadir'}"><img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Añadir'}" width="10" height="10" /></a><br />
								{if $product.minimal_quantity < ($customization.quantity -$quantityDisplayed) OR $product.minimal_quantity <= 1}
								<a rel="nofollow" class="cart_quantity_down" id="cart_quantity_down_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery}&amp;id_customization={$id_customization}&amp;op=down&amp;token={$token_cart}")}" title="{l s='Sustraer'}">
									<img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Sustraer'}" width="10" height="10" />
								</a>
								{else}
								<a class="cart_quantity_down" style="opacity: 0.3;" id="cart_quantity_down_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" href="#" title="{l s='Sustraer'}">
									<img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Sustraer'}" width="10" height="10" />
								</a>
								{/if}
		...
		{foreach $discounts as $discount}
			<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				...
				<td class="price_discount_del">
					{if strlen($discount.code)}<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" title="{l s='Borrar'}">{l s='Borrar'}</a>{/if}
				</td>
			</tr>
		{/foreach}
		</tbody>
...
<p>
	...
	<label for="allow_seperated_package">{l s='Enviar los productos disponibles primero'}</label>
</p>
{/if}
{if !$opc}
	{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
		<p>
			...
			<label for="enable-multishipping">{l s='Me gustaría especificar una dirección de envío individual para cada producto.'}</label>
		</p>
	{/if}
{/if}
<div id="cart_voucher" class="cart_voucher">
	{if $voucherAllowed}
		...
		<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
			<fieldset>
				<p class="title_block"><label for="discount_name">{l s='Vales'}</label></p>
		...
		{if $displayVouchers}
			<p id="title" class="title_offers">{l s='Aproveche nuestras ofertas exclusivas:'}</p>
...
<div class="order_delivery clearfix">
	{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
		...
		<ul id="delivery_address" class="address item">
			<li class="address_title">{l s='Dirección de entrega'}&nbsp;<span class="address_alias">({$delivery->alias})</span></li>
		...
		<ul id="invoice_address" class="address alternate_item">
			<li class="address_title">{l s='Dirección de facturación'}&nbsp;<span class="address_alias">({$invoice->alias})</span></li>
	...
	{else}
		{foreach from=$formattedAddresses key=k item=address}
			<ul class="address {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if}">
				<li class="address_title">{if $k eq 'invoice'}{l s='Dirección de facturación'}{elseif $k eq 'delivery' && $delivery->id}{l s='Dirección de entrega'}{/if}{if isset($address.object.alias)}&nbsp;<span class="address_alias">({$address.object.alias})</span>{/if}</li>
...
<p class="cart_navigation">
	{if !$opc}
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="exclusive standard-checkout" title="{l s='Siguiente'}">{l s='Siguiente'} &raquo;</a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="multishipping-button multishipping-checkout exclusive" title="{l s='Siguiente'}">{l s='Siguiente'} &raquo;</a>
		{/if}
	{/if}
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button_large" title="{l s='Seguir comprando'}">&laquo; {l s='Seguir comprando'}</a>
</p>
