...
{capture name=path}{l s='Sus direcciones'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1>{l s='Sus direcciones'}</h1>

<h3>
{if isset($id_address) && (isset($smarty.post.alias) || isset($address->alias))}
	{l s='Modificar la dirección'} 
	...
	{/if}
{else}
	{l s='Para añadir una nueva dirección, por favor rellene el siguiente formulario.'}
{/if}
</h3>
...
<form action="{$link->getPageLink('address', true)|escape:'html'}" method="post" class="std" id="add_address">
	<fieldset>
		<h3>{if isset($id_address)}{l s='Su dirección'}{else}{l s='Nueva dirección'}{/if}</h3>
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		{*
			si el cliente no ha actualizar su dirección de diseño, país tiene que ser verificado 
pero está en desuso
		*}
		{/if}
		...
		{if isset($one_phone_at_least) && $one_phone_at_least}
			<p class="inline-infos required">{l s='Usted debe registrar al menos un número de teléfono.'}</p>
		{/if}
		<p class="text">
		...
		<p class="required text" id="adress_alias">
			<label for="alias">{l s='Asigne un título de direcciones para futuras consultas.'} <sup>*</sup></label>
