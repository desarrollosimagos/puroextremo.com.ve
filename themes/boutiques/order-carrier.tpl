...
{if !$opc}
	...
	var msg = "{l s='Usted debe aceptar los términos del servicio antes de continuar.' js=1}";
	{literal}
	function acceptCGV()
	{
...
{if !$opc}
	{capture name=path}{l s='Envío:'}{/capture}
...
{if !$opc}
	<h1>{l s='Envío:'}</h1>
{else}
	<h2><span>2</span> {l s='Formas de entrega'}</h2>
{/if}
...
{else}
	<h3 class="carrier_title">{l s='Elija el método de entrega'}</h3>
	...
	{if isset($isVirtualCart) && $isVirtualCart}
		<p class="warning">{l s='No se necesita transporte para este pedido.'}</p>
	{else}
		{if $recyclablePackAllowed}
			<p class="checkbox">
				<input type="checkbox" name="recyclable" id="recyclable" value="1" {if $recyclable == 1}checked="checked"{/if} autocomplete="off"/>
				<label for="recyclable">{l s='Me gustaría recibir mi pedido en envases reciclados.'}.</label>
			</p>
		{/if}
		...
		{foreach $delivery_option_list as $id_address => $option_list}
			<h3>
				{if isset($address_collection[$id_address])}
					{l s='Elija una opción de envío para esta dirección:'} {$address_collection[$id_address]->alias}
				{else}
					{l s='Elija una opción de envío'}
				{/if}
			</h3>
							...
							<tr>
								...
								{if count($option_list) > 1}
									{if $option.is_best_grade}
										...
										<div class="delivery_option_best delivery_option_icon">{l s='El mejor precio y velocidad'}</div>
										{else}
										<div class="delivery_option_fast delivery_option_icon">{l s='El más rápido'}</div>
										{/if}
									{else}
										{if $option.is_best_price}
										<div class="delivery_option_best_price delivery_option_icon">{l s='El mejor precio'}</div>
										{/if}
									{/if}
								{/if}
								</td>
								...
								<div class="delivery_option_price">
									{if $option.total_price_with_tax && (!isset($free_shipping) || (isset($free_shipping) && !$free_shipping))}
										{if $use_taxes == 1}
											{if $priceDisplay == 1}
												{convertPrice price=$option.total_price_without_tax} {l s='(sin iva.)'}
											{else}
												{convertPrice price=$option.total_price_with_tax} {l s='(con iva.)'}
											{/if}
										{else}
								...
								<td {if $option.unique_carrier}class="first_item" colspan="2"{/if}>
									...
									{if isset($carrier.instance->delay[$cookie->id_lang])}
										...
										{if count($carrier.product_list) <= 1}
											({l s='Producto afectado:'}
										{else}
											({l s='Productos afectados:'}
										{/if}
			...
			<p class="warning" id="noCarrierWarning">
				{foreach $cart->getDeliveryAddressesWithoutCarriers(true) as $address}
					{if empty($address->alias)}
						{l s='Ningún proveedor disponible.'}
					{else}
						{l s='Ningún proveedor disponible para la dirección "%s".' sprintf=$address->alias}
					{/if}
					{if !$address@last}
					<br />
					{/if}
	...
	<div style="display: none;" id="extra_carrier"></div>
		...
		{if $giftAllowed}
		<h3 class="gift_title">{l s='Regalo'}</h3>
		<p class="checkbox">
			<input type="checkbox" name="gift" id="gift" value="1" {if $cart->gift == 1}checked="checked"{/if} autocomplete="off"/>
			<label for="gift">{l s='Me gustaría que mi pedido sea envuelto como regalo.'}</label>
			<br />
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			{if $gift_wrapping_price > 0}
				({l s='Costo adicional de'}
				<span class="price" id="gift-price">
					{if $priceDisplay == 1}{convertPrice price=$total_wrapping_tax_exc_cost}{else}{convertPrice price=$total_wrapping_cost}{/if}
				</span>
				{if $use_taxes}{if $priceDisplay == 1} {l s='(sin iva.)'}{else} {l s='(iva.)'}{/if}{/if})
			{/if}
		</p>
		<p id="gift_div" class="textarea">
			<label for="gift_message">{l s='Si desea, puede agregar una nota al regalo:'}</label>
...
{if $conditions AND $cms_id}
	<h3 class="condition_title">{l s='Términos del servicio'}</h3>
	<p class="checkbox">
		<input type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} autocomplete="off"/>
		<label for="cgv">{l s='Estoy de acuerdo con los términos del servicio y voy a respetar incondicionalmente.'}</label> <a href="{$link_conditions}" class="iframe">{l s='(Leer los Términos del servicio)'}</a>
	</p>
	<script type="text/javascript">
		$(document).ready(function() {
	        $("a.iframe").fancybox({
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if !$is_guest}
			{if $back}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Anterior'}" class="button">&laquo; {l s='Anterior'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Anterior'}" class="button">&laquo; {l s='Anterior'}</a>
			{/if}
		{else}
				<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Anterior'}" class="button">&laquo; {l s='Anterior'}</a>
		{/if}
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Siguiente'} &raquo;" class="exclusive" />
		{/if}
	</p>
</form>
{else}
	<h3>{l s='Deja un mensaje'}</h3>
	<div>
