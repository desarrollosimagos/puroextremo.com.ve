...
// Translations
var doesntExist = '{l s='Esta combinación no existe para este producto. Por favor, seleccione otra combinación.' js=1}';
var doesntExistNoMore = '{l s='Este producto ya no está disponible' js=1}';
var doesntExistNoMoreBut = '{l s='con esos atributos pero está disponible con otros.' js=1}';
var uploading_in_progress = '{l s='Carga en curso, por favor sea paciente.' js=1}';
var fieldRequired = '{l s='Por favor, rellene todos los campos requeridos antes de guardar su personalización.' js=1}';
...
<div id="primary_block" class="clearfix">
	...
	<div id="admin-action">
		<p>{l s='Este producto no es visible para sus clientes.'}
		<input type="hidden" id="admin-action-product-id" value="{$product->id}" />
		<input type="submit" value="{l s='Publicar'}" class="exclusive" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 0, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
		<input type="submit" value="{l s='Atrás'}" class="exclusive" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 1, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
		</p>
	...
	<div id="pb-right-column">
		...
		{if $have_image}
			<span id="view_full_size">
				...
				{if $product->on_sale}
					<img src="{$img_dir}onsale_{$lang_iso}.gif" alt="{l s='en venta'}" class="on_sale_img"/>
				...
				{/if}
				<span class="span_link">{l s='Ver tamaño completo'}</span>
			</span>
		{else}
			<span id="view_full_size">
		...
		{/if}
		{if isset($images) && count($images) > 1}<p class="resetimg clear"><span id="wrapResetImages" style="display: none;"><a id="resetImages" href="{$link->getProductLink($product)}" onclick="$('span#wrapResetImages').hide('slow');return (false);">{l s='Ver todas las imágenes'}</a></span></p>{/if}
		<!-- usefull links-->
		<ul id="usefull_link_block">
			{if $HOOK_EXTRA_LEFT}{$HOOK_EXTRA_LEFT}{/if}
			<li class="print"><a href="javascript:print();">{l s='Imprimir'}</a></li>
	...
	<div id="pb-left-column">
		...
		<div class="content_prices clearfix">
			...
			<p class="online_only">{l s='Solo por Internet'}</p>
			{/if}
				...
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					{if $productPriceWithoutReduction > $productPrice}
						...
						<!-- {if $tax_enabled && $display_tax_label == 1}
							{if $priceDisplay == 1}{l s='sin iva.'}{else}{l s='iva incl.'}{/if}
						{/if} -->
					{/if}
				{/if}
				</span>
			...
			{/if}	
				...
				{if $priceDisplay == 2}
					<span id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='sin iva.'}</span>
				{/if}
			...
			{if $packItems|@count && $productPrice < $product->getNoPackPrice()}
				<p class="pack_price">{l s='en lugar de'} <span style="text-decoration: line-through;">{convertPrice price=$product->getNoPackPrice()}</span></p>
				<br class="clear" />
			{/if}
			{if $product->ecotax != 0}
				<p class="price-ecotax">{l s='Incluir'} <span id="ecotax_price_display">{if $priceDisplay == 2}{$ecotax_tax_exc|convertAndFormatPrice}{else}{$ecotax_tax_inc|convertAndFormatPrice}{/if}</span> {l s='Para el impuesto ecologico'}
					{if $product->specificPrice AND $product->specificPrice.reduction}
					<br />{l s='(No incluido en el descuento)'}
					{/if}
				</p>
			{/if}
			{if !empty($product->unity) && $product->unit_price_ratio > 0.000000}
				 {math equation="pprice / punit_price"  pprice=$productPrice  punit_price=$product->unit_price_ratio assign=unit_price}
				<p class="unit-price"><span id="unit_price_display">{convertPrice price=$unit_price}</span> {l s='para'} {$product->unity|escape:'htmlall':'UTF-8'}</p>
			{/if}
		...
		<div id="short_description_block">
			...
			<p class="buttons_bottom_block"><a href="javascript:{ldelim}{rdelim}" class="button">{l s='Más detalles'}</a></p>
			{/if}
			{if $packItems|@count > 0}
			<div class="short_description_pack">
				<h3>{l s='Contenido del paquete'}</h3>
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
				<label for="product_reference">{l s='Referencia:'} </label>
			...
			<p id="minimal_quantity_wanted_p"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				{l s='Este producto no se vende individualmente. Debe seleccionar al menos'} <b id="minimal_quantity_label">{$product->minimal_quantity}</b> {l s='unidades para este producto.'}
			</p>
			...
			<p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<span id="availability_label">{l s='Disponibilidad:'}</span>
				<span id="availability_value"{if $product->quantity <= 0} class="warning_inline"{/if}>
				{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='Este producto ya no está disponible'}{/if}{else}{$product->available_now}{/if}
				</span>
			</p>
			...
			<p id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
				...
				<span {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s='artículo disponible'}</span>
				<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='artículo disponible'}</span>
			</p>
			{/if}

			<p class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Advertencia: ¡Últimos artículos en stock!'}</p>
		</div>
			<!-- quantity wanted -->			
			
			<div class="group_button_add">
				...
				<a rel="nofollow" class="cart_quantity_up" id="" href="javascript:void(0)" title="{l s='Añadir'}">
				<img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Añadir'}" width="10" height="10" /></a>
				<a rel="nofollow" class="cart_quantity_down" id="" href="javascript:void(0)" title="{l s='Sustraer'}">
					<img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Sustraer'}" width="10" height="10" />
				</a>
				</span>
				</p>
			...
			{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE}
				<span class="exclusive">
					<span></span>
					{l s='Añadir a la cesta'}
				</span>
			{else}
				<p id="add_to_cart" class="buttons_bottom_block">
					<input type="submit" name="Submit" value="{l s='Añadir a la cesta'}" class="exclusive" />
				</p>
			{/if}
...
<ul class="idTabs clearfix">
	<li><a href="#discount" style="cursor: pointer" class="selected">{l s='Fijación de precios a escala móvil'}</a></li>
</ul>
...
<div id="more_info_block" class="clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
		{if $product->description}<li><a id="more_info_tab_more_info" href="#idTab1">{l s='Más info'}</a></li>{/if}
		{if $features}<li><a id="more_info_tab_data_sheet" href="#idTab2">{l s='Hoja de datos'}</a></li>{/if}
		{if $attachments}<li><a id="more_info_tab_attachments" href="#idTab9">{l s='Descargar'}</a></li>{/if}
		{if isset($accessories) AND $accessories}<li><a href="#idTab4">{l s='Accesorios'}</a></li>{/if}
		{if isset($product) && $product->customizable}<li><a href="#idTab10">{l s='Personalización del producto'}</a></li>{/if}
		{$HOOK_PRODUCT_TAB}
	</ul>
	<div id="more_info_sheets" class="sheets align_justify not-animated"  data-animate="fadeInUp" data-delay="200">
	{if $product->description}<div class="title_hide_show" style="display:none">{l s='Mas info'}</div>{/if}
	{if isset($product) && $product->description}
		<!-- full description -->
		<div id="idTab1" class="rte content_hide_show">{$product->description}</div>
	{/if}
	{if $features}<div class="title_hide_show" style="display:none">{l s='Hoja de datos'}</div>{/if}
	...
	{/if}
	{if $attachments}<div class="title_hide_show" style="display:none">{l s='Descargar'}</div>{/if}
	...
	{/if}
	{if isset($accessories) AND $accessories}<div class="title_hide_show" style="display:none">{l s='Accesorios'}</div>{/if}
				...
				<div class="block_content">
					...
					{foreach from=$accessories item=accessory name=accessories_list}
						{if ($accessory.allow_oosp || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="{if isset($grid_product)}{$grid_product}{else}grid_5{/if} ajax_block_product {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description clearfix">
	...
	{if isset($product) && $product->customizable}<div class="title_hide_show" style="display:none">{l s='Personalización del producto'}</div>{/if}
	{if isset($product) && $product->customizable}
		<div id="idTab10" class="bullet customization_block content_hide_show">
			<form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
				<p class="infoCustomizable">
					{l s='Después de guardar su producto personalizado, no olvide añadirlo a su carrito.'}
					{if $product->uploadable_files}<br />{l s='Formatos de archivo permitidos: GIF, JPG, PNG'}{/if}
				</p>
							...
							{if $field.type == 0}
								<li class="customizationUploadLine{if $field.required} required{/if}">{assign var='key' value='pictures_'|cat:$product->id|cat:'_'|cat:$field.id_customization_field}
									...
									<div class="customizationUploadBrowse">
										...
										<a href="{$link->getProductDeletePictureLink($product, $field.id_customization_field)}" title="{l s='Borrar'}" >
											<img src="{$img_dir}icon/delete.gif" alt="{l s='Borrar'}" class="customization_delete_icon" width="12" height="12" />
										</a>
									</div>
									{/if}
									<div class="customizationUploadBrowse">
										<label class="customizationUploadBrowseDescription">{if !empty($field.name)}{$field.name}{else}{l s='Por favor, seleccione un archivo de imagen de su disco duro'}{/if}{if $field.required}<sup>*</sup>{/if}</label>
				...
				<p id="customizedDatas">
					...
					<input type="button" class="button" value="{l s='Guardar'}" onclick="javascript:saveCustomization()" />
					<span id="ajax-loader" style="display:none"><img src="{$img_ps_dir}loader.gif" alt="loader" /></span>
				</p>
			</form>
			<p class="clear required"><sup>*</sup> {l s='campos requeridos'}</p>
		</div>
	{/if}
...
{if isset($packItems) && $packItems|@count > 0}
	<div id="blockpack">
		<h2>{l s='Contenido del paquete'}</h2>
