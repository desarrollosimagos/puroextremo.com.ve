...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
	<div>
		...
		<p class="title_block">
			<input type="submit" value="{l s='Reordenar'}" name="submitReorder" class="button exclusive" />
			{l s='Datos de pedido %s - colocado en' sprintf=$order->getUniqReference()} {dateFormat date=$order->date_add full=0}
		</p>
	</div>
</form>

<div class="info-order">
{if $carrier->id}<p><strong>{l s='Portador'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'htmlall':'UTF-8'}{else}{$carrier->name|escape:'htmlall':'UTF-8'}{/if}</p>{/if}
<p><strong>{l s='Forma de pago'}</strong> <span class="color-myaccount">{$order->payment|escape:'htmlall':'UTF-8'}</span></p>
{if $invoice AND $invoiceAllowed}
<p>
	<img src="{$img_dir}icon/pdf.gif" alt="" class="icon" />
	<a target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">{l s='Descarga tu factura como un archivo PDF.'}</a>
</p>
{/if}
{if $order->recyclable}
<p><img src="{$img_dir}icon/recyclable.gif" alt="" class="icon" />&nbsp;{l s='Usted ha dado permiso para recibir su pedido en envases reciclados.'}</p>
{/if}
{if $order->gift}
	<p><img src="{$img_dir}icon/gift.gif" alt="" class="icon" />&nbsp;{l s='Usted ha solicitado la envoltura de regalos para este pedido.'}</p>
	<p>{l s='Mensaje'} {$order->gift_message|nl2br}</p>
{/if}
</div>

{if count($order_history)}
<h3>{l s='Seguir el estado de su pedidon paso a paso.'}</h3>
...
{if isset($followup)}
<p class="bold">{l s='Haga clic en el siguiente enlace para seguir la entrega de su pedido'}</p>
...
<ul class="address item {if $order->isVirtual()}full_width{/if}">
	<li class="address_title">{l s='Facturación'}</li>
	...
	<table class="std">
		<thead>
			<tr>
				...
				<th class="{if $return_allowed}item{else}first_item{/if}">{l s='Referencia'}</th>
				<th class="item">{l s='Producto'}</th>
				<th class="item">{l s='Cantidad'}</th>
				{if $order->hasProductReturned()}
					<th class="item">{l s='Devuelto'}</th>
				{/if}
				<th class="item">{l s='Precio por unidad'}</th>
		...
		<tfoot>
			{if $priceDisplay && $use_tax}
				<tr class="item">
					<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
						{l s='Total productos (iva excl.)'} <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithoutTaxes() currency=$currency}</span>
					</td>
				</tr>
			{/if}
			<tr class="item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					{l s='Total productos'} {if $use_tax}{l s='(iva incl.)'}{/if}: <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
				</td>
			</tr>
			{if $order->total_discounts > 0}
			<tr class="item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					{l s='Vales totales:'} <span class="price-discount">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
				</td>
			</tr>
			{/if}
			...
			<tr class="item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					{l s='Costo total envoltura de regalos:'} <span class="price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
				</td>
			</tr>
			{/if}
			<tr class="item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					{l s='Total de envío'} {if $use_tax}{l s='(iva incl.)'}{/if}: <span class="price-shipping">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
				</td>
			</tr>
						...
						<td class="bold">
							<label for="cb_{$product.id_order_detail|intval}">
								{if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0}
									...
									<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&amp;id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html'}" title="{l s='Descarga este producto'}">
									{else}
										<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")|escape:'html'}" title="{l s='Descarga este producto'}">
									{/if}
										<img src="{$img_dir}icon/download_product.gif" class="icon" alt="{l s='Descarga este producto'}" />
									</a>
									{if isset($is_guest) && $is_guest}
										<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html'}" title="{l s='Descarga este producto'}"> {$product.product_name|escape:'htmlall':'UTF-8'} 	</a>
									{else}
									<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")|escape:'html'}" title="{l s='Descarga este producto'}"> {$product.product_name|escape:'htmlall':'UTF-8'} 	</a>
									{/if}
								{else}
		...
		{foreach from=$discounts item=discount}
			<tr class="item">
				...
				<td>{l s='Vales'} {$discount.name|escape:'htmlall':'UTF-8'}</td>
...
{if $order->getShipping()|count > 0}
	<table class="std">
		<thead>
			<tr>
				<th class="first_item">{l s='Fecha'}</th>
				<th class="item">{l s='Transporte'}</th>
				<th class="item">{l s='Peso'}</th>
				<th class="item">{l s='Costo de envío'}</th>
				<th class="last_item">{l s='Número de seguimiento'}</th>
			</tr>
		</thead>
		<tbody>
...
{if !$is_guest}
	...
	<div id="returnOrderMessage">
		<h3>{l s='Devolución de Mercancía'}</h3>
		<p>{l s='Si desea devolver uno o más productos, por favor marque la casilla correspondiente y proporcionar una explicación para el cambio. Cuando haya terminado, haga clic en el botón de abajo.'}</p>
		<p class="textarea">
			<textarea cols="67" rows="3" name="returnText"></textarea>
		</p>
		<p class="submit">
			<input type="submit" value="{l s='Hacer un deslizamiento RMA'}" name="submitReturnMerchandise" class="button_large" />
	...
	<div class="table_block">
		<table class="detail_step_by_step std">
			<thead>
				<tr>
					<th class="first_item" style="width:150px;">{l s='Desde'}</th>
					<th class="last_item">{l s='Mensaje'}</th>
				</tr>
			</thead>
			<tbody>
	...
	{if isset($errors) && $errors}
		<div class="error">
			<p>{if $errors|@count > 1}{l s='Hay %d errores' sprintf=$errors|@count}{else}{l s='Hay un %d error' sprintf=$errors|@count}{/if}</p>
			<ol>
	...
	<p class="success">
		{l s='Mensaje enviado con éxito'}
	</p>
	{/if}
	<form action="{$link->getPageLink('order-detail', true)|escape:'html'}" method="post" class="std" id="sendOrderMessage">
		<h3>{l s='Añadir un mensaje'}</h3>
		<p>{l s='Si desea agregar un comentario acerca de su pedido, por favor escríbalo en el campo de abajo.'}</p>
		<p>
		<label for="id_product">{l s='Producto'}</label>
			<select name="id_product" style="width:300px;">
				<option value="0">{l s='-- Elegir --'}</option>
				{foreach from=$products item=product name=products}
		...
		<p class="submit">
			...
			<input type="submit" class="button" name="submitMessage" value="{l s='Enviar'}"/>
		</p>
	</form>
{else}
<p><img src="{$img_dir}icon/infos.gif" alt="" class="icon" />&nbsp;{l s='No se puede devolver la mercancía con una cuenta de invitado'}</p>
{/if}
{/if}