...
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html'}">{l s='Mi cuenta'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Mis direcciones'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1>{l s='Mis direcciones'}</h1>
<p>{l s='Por favor, configure su facturación por defecto y direcciones de entrega al hacer un pedido. También puede añadir direcciones adicionales, lo que puede ser útil para el envío de regalos o recibir una orden en su oficina.'}</p>

{if isset($multipleAddresses) && $multipleAddresses}
<div class="addresses">
	<h3>{l s='Sus direcciones son las siguientes.'}</h3>
	<p>{l s='Asegúrese de actualizar su información personal si ha cambiado.'}</p>
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
			...
			{/foreach}
			<li class="address_update"><a href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html'}" title="{l s='Update'}">&raquo; {l s='Actualizar'}</a></li>
			<li class="address_delete"><a href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}">&raquo; {l s='Borrar'}</a></li>
		</ul>
	{/foreach}
	</div>
	<p class="clear" />
...
{else}
	<p class="warning">{l s='No hay direcciones disponibles.'}&nbsp;<a href="{$link->getPageLink('address', true)|escape:'html'}">{l s='Añadir una nueva dirección'}</a></p>
{/if}

<div class="clear address_add"><a href="{$link->getPageLink('address', true)|escape:'html'}" title="{l s='Add an address'}" class="button_large">{l s='Añadir una dirección'}</a></div>
