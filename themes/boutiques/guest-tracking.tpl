...
{if isset($order_collection)}
	...
	<h2 id="guestToCustomer">{l s='Para más ventajas...'}</h2>

	{include file="$tpl_dir./errors.tpl"}
	
	{if isset($transformSuccess)}
		<p class="success">{l s='Su cuenta de invitado se ha transformado con éxito en una cuenta de cliente. Ahora puede iniciar sesión como un comprador registrado. '} <a href="{$link->getPageLink('authentication', true)|escape:'html'}">{l s='pagina.'}</a></p>
	{else}
		<form method="post" action="{$action|escape:'htmlall':'UTF-8'}#guestToCustomer" class="std">
			<fieldset class="description_box">
				<p class="bold">{l s='Transforme su cuenta de invitado en una cuenta de clientes y disfrutar de:'}</p>
				<ul class="bullet">
					<li>{l s='Acceso personalizado y seguro'}</li>
					<li>{l s='Pedido Rápido y fácil'}</li>
					<li>{l s='Más fácil la devolución de mercancía'}</li>
				</ul>
				<p class="text">
					<label>{l s='Establezca la contraseña:'}</label>
...
{else}
	...
	{if isset($show_login_link) && $show_login_link}
		<p><img src="{$img_dir}icon/userinfo.gif" alt="{l s='Information'}" class="icon" /><a href="{$link->getPageLink('my-account', true)|escape:'html'}">{l s='Haga clic aquí para ingresar a su cuenta de cliente.'}</a><br /><br /></p>
	{/if}
	<form method="post" action="{$action|escape:'htmlall':'UTF-8'}" class="std">
		<fieldset class="description_box">
			<p>{l s='Para realizar el seguimiento de su pedido, por favor ingrese la siguiente información:'}</p>
			<p class="text">
				<label>{l s='Datos de pedido:'} </label>
				...
				<i>{l s='Por ejemplo: QIIXJXNUI o QIIXJXNUI#1'}</i>
			</p>
			...
			<p class="center"><input type="submit" class="button" name="submitGuestTracking" value="{l s='Enviar'}" /></p>
		</fieldset>
	</form>
{/if}
