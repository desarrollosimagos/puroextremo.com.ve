...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">{l s='Anterior'}</a></li>
		{else}
			<li id="pagination_previous" class="disabled"><span>{l s='Anterior'}</span></li>
		{/if}
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{l s='Siguiente'}</a></li>
		{else}
			<li id="pagination_next" class="disabled"><span>{l s='Siguiente'}</span></li>
		{/if}
		</ul>
	{/if}
