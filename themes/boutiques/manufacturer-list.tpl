...
{else}
	<p class="nbrmanufacturer">{strip}
		<span class="bold">
			{if $nbManufacturers == 0}{l s='No hay fabricantes.'}
			{else}
				{if $nbManufacturers == 1}
					{l s='Hay un %d fabricante.' sprintf=$nbManufacturers}
				{else}
					{l s='Hay %d fabricantes.' sprintf=$nbManufacturers}
				{/if}
			{/if}
		</span>{/strip}
	</p>
		...
		{foreach from=$manufacturers item=manufacturer name=manufacturers}
			<li class="clearfix {if $smarty.foreach.manufacturers.first}first_item{elseif $smarty.foreach.manufacturers.last}last_item{else}item{/if}"> 
				<div class="left_side">
					...
					<br />
					{if $manufacturer.nb_products > 0}<a href="{$link->getmanufacturerLink($manufacturer.id_manufacturer, $manufacturer.link_rewrite)|escape:'htmlall':'UTF-8'}">{/if}
						<span>{if $manufacturer.nb_products == 1}{l s='%d producto' sprintf=$manufacturer.nb_products|intval}{else}{l s='%d productos' sprintf=$manufacturer.nb_products|intval}{/if}</span>
					{if $manufacturer.nb_products > 0}</a>{/if}
					</p>
				</div>
				...
				{if $manufacturer.nb_products > 0}
					<a class="button" href="{$link->getmanufacturerLink($manufacturer.id_manufacturer, $manufacturer.link_rewrite)|escape:'htmlall':'UTF-8'}">{l s='ver producto'}</a>
				{/if}
				</div>
			</li>
		{/foreach}
