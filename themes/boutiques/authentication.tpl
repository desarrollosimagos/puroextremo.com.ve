...
{capture name=path}
	{if !isset($email_create)}{l s='Login'}{else}
		<a href="{$link->getPageLink('authentication', true)|escape:'html'}" rel="nofollow" title="{l s='Authentication'}">{l s='Autenticación'}</a>
...
<h1>{if !isset($email_create)}{l s='Iniciar Sesión'}{else}{l s='Crear una cuenta'}{/if}</h1>
...
{literal}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="create-account_form" class="std">
		<fieldset>
			<h3>{l s='Crear una cuenta'}</h3>
			<div class="form_content clearfix">
				<p class="title_block">{l s='Introduzca su dirección de correo electrónico para crear una cuenta.'}</p>
				<div class="error" id="create_account_error" style="display:none"></div>
				<p class="text">
					<label for="email_create">{l s='Dirección de correo electrónico'}</label>
				...
				<p class="submit">
					...
					<input type="hidden" class="hidden" name="SubmitCreate" value="{l s='Crear una cuenta'}" />
				</p>
			</div>
		</fieldset>
	</form>
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="login_form" class="std">
		<fieldset>
			...
			<div class="form_content clearfix">
				<p class="text">
					<label for="email">{l s='Dirección de correo electrónico'}</label>
				...
				<p class="text">
					<label for="passwd">{l s='Contraseña'}</label>
				...
				<p class="submit">
					...
					<input type="submit" id="SubmitLogin" name="SubmitLogin" class="button" value="{l s='Iniciar la Sesión'}" />
				</p>				
				<p class="lost_password"><a href="{$link->getPageLink('password')}">{l s='¿Olvidaste tu contraseña?'}</a></p>
			</div>
		</fieldset>
	</form>
	...
	<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html'}" method="post" id="new_account_form" class="std clearfix">
		<fieldset>
			<h3>{l s='Pago y envío instantáneo'}</h3>
			<div id="opc_account_form" style="display: block; ">
				<!-- Account -->
				<p class="required text">
					<label for="guest_email">{l s='Dirección de correo electrónico'} <sup>*</sup></label>
				...
				{foreach from=$dlv_all_fields item=field_name}
					...
					{elseif $field_name eq "State:name"}
						...
						<p class="required id_state select">
							<label for="id_state">{l s='Estados'} <sup>*</sup></label>
		...
		<p class="cart_navigation required submit">
			<span><sup>*</sup>{l s='campos obligatorios'}</span>
...
<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
		<h3>{l s='Su información personal'}</h3>
		<p class="radio required">
		...
		<p class="required text">
			<label for="customer_firstname">{l s='Nombres'} <sup>*</sup></label>
		...
		<p class="required text">
			<label for="customer_lastname">{l s='Apellidos'} <sup>*</sup></label>
		...
		<p class="required password">
			...
			<span class="form_info">{l s='(Cinco caracteres mínimos)'}</span>
		</p>
		<p class="select">
			<span>{l s='Fecha de nacimiento'}</span>
			<select id="days" name="days">
	...
	{if $b2b_enable}
	<fieldset class="account_creation">
		<h3>{l s='Información de su empresa'}</h3>
		<p class="text">
			<label for="">{l s='Empresa'}</label>
			<input type="text" class="text" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
		</p>
		<p class="text">
		...
		<p class="text">
			<label for="website">{l s='sitio web'}</label>
	...
	<fieldset class="account_creation">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "address1"}
				<p class="required text">
					<label for="address1">{l s='Dirección'} <sup>*</sup></label>
					<input type="text" class="text" name="address1" id="address1" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{/if}" />
					<span class="inline-infos">{l s='Dirección, P.O. Caja, nombre de la empresa, etc'}</span>
				</p>
			{elseif $field_name eq "address2"}
				<p class="text">
					...
					<span class="inline-infos">{l s='Departamento, habitación, unidad, edificio, piso, etc ..'}</span>
				</p>
			{elseif $field_name eq "postcode"}
			{assign var='postCodeExist' value=true}
				<p class="required postcode text">
		...
		{if isset($one_phone_at_least) && $one_phone_at_least}
			<p class="inline-infos">{l s='Usted debe registrar al menos un número de teléfono.'}</p>
		{/if}
		<p class="text">
		...
		<p class="required text" id="address_alias">
			<label for="alias">{l s='Asigne un alias de direcciones para futuras consultas.'} <sup>*</sup></label>
	...
	<fieldset class="account_creation dni">
		<h3>{l s='Número de identificación'}</h3>
		<p class="required text">
			<label for="dni">{l s='Número de identificación'} <sup>*</sup></label>
			<input type="text" class="text" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{/if}" />
			<span class="form_info">{l s='DNI / NIF / NIE'}</span>
