...
<h1>{l s='Servicio al cliente'} - {if isset($customerThread) && $customerThread}{l s='Tu respuesta'}{else}{l s='Contáctenos'}{/if}</h1>

{if isset($confirmation)}
	<p>{l s='Tu mensaje ha sido enviado con éxito a nuestro equipo.'}</p>
	<ul class="footer_links">
		<li><a href="{$base_dir}"><img class="icon" alt="" src="{$img_dir}icon/home.gif"/></a><a href="{$base_dir}">{l s='Inicio'}</a></li>
	</ul>
{elseif isset($alreadySent)}
	<p>{l s='Tu mensaje ha sido enviado.'}</p>
	<ul class="footer_links">
		<li><a href="{$base_dir}"><img class="icon" alt="" src="{$img_dir}icon/home.gif"/></a><a href="{$base_dir}">{l s='Inicio'}</a></li>
	</ul>
{else}
	<p class="bold">{l s='Para preguntas relacionadas con un pedido o para obtener más información acerca de nuestros productos'}.</p>
	{include file="$tpl_dir./errors.tpl"}
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
			<h3>{l s='enviar un mensaje'}</h3>
			<p class="select">
				<label for="id_contact">{l s='Asunto'}</label>
		...
		{if !$PS_CATALOG_MODE}
			...
			<p class="text select">
				<label for="id_order">{l s='Referencia de pedido'}</label>
			...
			<p class="text select">
			<label for="id_product">{l s='Productos'}</label>
