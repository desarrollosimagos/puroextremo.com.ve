...
// <![CDATA[
	...
	{
		return {
			'invoice': "{l s='Tu dirección de facturación' js=1}",
			'delivery': "{l s='Tu dirección de entrega' js=1}"
		};
	}
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}" title="{l s='Actualizar' js=1}">&raquo; {l s='Actualizar' js=1}</a>';
...
{if !$opc}
{capture name=path}{l s='Direcciones'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}
{/if}

{if !$opc}<h1>{l s='Direcciones'}</h1>{else}<h2><span>1</span> {l s='Direcciones'}</h2>{/if}
...
{if !$opc}
	...
	{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="button_multishipping_mode" id="multishipping_mode_box">
			<div class="title">{l s='Multi-envío'}</div>
			<div class="description">
				<a href="{$link->getPageLink('order', true, NULL, 'step=1&multi-shipping=1')|escape:'html'}"/>
					{l s='Especifique una dirección de entrega para cada producto solicitado.'}
				</a>
			</div>
		</div>
	{/if}
	...
	{if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="address-form-multishipping">
			<div class="button_multishipping_mode" id="multishipping_mode_box">
				<div class="title">{l s='Multi-envío'}</div>
				<div class="description">
					<input type="checkbox" id="multishipping_mode_checkbox" onchange="multishippingMode(this); return false;" autocomplete="off"/><label for="multishipping_mode_checkbox">{l s='Me gustaría especificar una dirección de envío para cada producto solicitado.'}</label>
				</div>
				<div class="description_off">
					<a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')|escape:'html'}" id="link_multishipping_form" title="{l s='Elija la dirección de entrega(s)'}">
						{l s='Especifique una dirección de entrega para cada producto.'}
					</a>
				</div>
			</div>
			<script type="text/javascript">
...
{/if}
	<div class="addresses clearfix">
		<p class="address_delivery select">
			<label for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Elija una dirección de facturación:'}{else}{l s='Elija una dirección de entrega:'}{/if}</label>
		...
		<p class="checkbox addressesAreEquals" {if $cart->isVirtualCart()}style="display:none;"{/if}>
			...
			<label for="addressesAreEquals">{l s='Utilice la dirección de entrega como la dirección de facturación.'}</label>
		</p>

		<p id="address_invoice_form" class="select" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>

		{if $addresses|@count > 1}
			<label for="id_address_invoice" class="strong">{l s='Elija una dirección de facturación:'}</label>
			...
			{else}
				<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Agregar'}" class="button_large">{l s='Añadir una nueva dirección'}</a>
			{/if}
		</p>
		<div class="clearfix">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Agregar'}" class="button_large">{l s='Añadir una nueva dirección'}</a>
		</p>
		{if !$opc}
		<div id="ordermsg" class="clearfix">
			<p class="txt">{l s='Si desea agregar un comentario acerca de su pedido, por favor escríbalo en el campo de abajo.'}</p>
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" title="{l s='Anterior'}" class="button">&laquo; {l s='Anterior'}</a>
		<input type="submit" name="processAddress" value="{l s='Siguiente'} &raquo;" class="exclusive" />
	</p>
</form>
{else}
</div>
