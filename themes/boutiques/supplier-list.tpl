...
{capture name=path}{l s='Proveedores:'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1>{l s='Proveedores:'}</h1>
...
{else}
	...
	<p class="nbrmanufacturer">{strip}
		<span class="bold">
			{if $nbSuppliers == 0}{l s='Hay no proveedores.'}
			{else}
				{if $nbSuppliers == 1}
					{l s='Hay %d proveedor.' sprintf=$nbSuppliers}
				{else}
					{l s='Hay %d proveedores.' sprintf=$nbSuppliers}
				{/if}
			{/if}
		</span>{/strip}
	</p>
		...
		<li class="clearfix {if $supplier@first}first_item{elseif $supplier@last}last_item{else}item{/if}">
			<div class="left_side">
				...
				{if $supplier.nb_products > 0}
					<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">
				{/if}
					<span>{if $supplier.nb_products == 1}{l s='%d producto' sprintf=$supplier.nb_products|intval}{else}{l s='%d productos' sprintf=$supplier.nb_products|intval}{/if}</span>
				{if $supplier.nb_products > 0}
					</a>
				{/if}
				</p>
			...
			{if $supplier.nb_products > 0}
				<a class="button" href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">{l s='Ver productos'}</a>
			{/if}
			</div>
		</li>
	{/foreach}
