...
$(document).ready(function(){
	...
	if (is_touch_device())
	{
		...
		$('#header').on('touchstart', '.shopping_cart a:first', function(){
...
function crossselling_serialScroll()
{
	...
	$(document).ready(function(){
		...
		var flexmincross = 1;
		var flexmaxcross = 5;
			if($('#layer_cart .crossseling-content').width()<=280){ flexmincross = 1;  flexmaxcross = 1; }
		...
		checkforloadedcats[selector] = setInterval(function() {
			...
			if (image.complete || image.readyState == 'complete' || image.readyState == 4) {
				...
				$(selector).flexslider({
					...
					controlNav: false,
					itemWidth:  238, 
					minItems: flexmincross,                   
					maxItems: flexmaxcross, 
					move: 0		});	
			}
		...
		$(window).resize(function() {
			...
			if($('#layer_cart .crossseling-content').width()<=280){ $(selector).data('flexslider').setOpts({minItems: 1, maxItems: 1});}
			...
			else if($('#layer_cart .crossseling-content').width()>=963){ $(selector).data('flexslider').setOpts({minItems: grid_size_md, maxItems: grid_size_md});}
