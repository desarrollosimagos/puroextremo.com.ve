...
<div class="yotpo bottomLine"
	...
	data-lang="{$yotpoLanguage|escape:'htmlall':'UTF-8'}"></div>
		{/if}  
			...
			{if !$content_only}<a itemprop="brand" itemscope="" itemtype="http://schema.org/Organization" href="{$link->getmanufacturerLink($product->id_manufacturer, $product_manufacturer->link_rewrite)}" title="{l s='All products of this manufacturer'}"> {/if}  <span itemprop="name" style="display: none">{$product_manufacturer->name}</span>
				<img itemprop="image" class="imglog" src="{$base_dir_ssl}img/m/{$product_manufacturer->id_manufacturer}-mf_image2.jpg" />{if !$content_only}</a>{/if}{else}<label>{l s='Manufacturer:'}</label> 
				{if !$content_only}<a itemprop="brand" itemscope="" class="manname" itemtype="http://schema.org/Organization" href="{$link->getmanufacturerLink($product->id_manufacturer, $product_manufacturer->link_rewrite)}" title="{l s='All products of this manufacturer'}">{/if}<span itemprop="name">{$product_manufacturer->name}</span>{if !$content_only}</a>{/if}{/if} {/if}
				...
				<div class="more_buttons_bottom_block no-print">
						{if isset($comparator_max_item) && $comparator_max_item}
							<div class="compare additional_button no-print">
								<a class="add_to_compare" href="{$product->link_rewrite|escape:'html':'UTF-8'}" data-id-product="{$product->id}" title="{l s='Add to Compare'}">{l s='Add to Compare'}</a>
							</div>
						{/if}
